window.addEventListener( `load`, function(){// Ao carregar a página.. no caso janela
  ...
  // Da mesma forma do que é para carregar tudo, é aqui, porém ele dispara com o click sobre  botão
	document.querySelector( `#Enviar` ).addEventListener( `click`, () => {// Arrow function, como comentei na linha 7, sem passar valor
		...
		let citar = document.querySelector(".citar").innerHTML
		// Ele irá sempre enviar o que ele está citando
	...
	websocket.onmessage = ( ev ) => {// sintaxe: Conexão.onmessage = (valor)
		...
		let citar = msg.quote === undefined ? "" : msg.quote// Capturando informações do json.
		...
		document.getElementById( `conteudos` ).innerHTML = `
			<div class="msg" ondblclick="quote('${uname}', '${umsg}', '.citar');">
				<blockquote>${citar}</blockquote><br/>
				<strong>${uname}</strong> : ${marked(umsg)}
			</div>`;// Este marked, é uma função, que transformará em markdown
